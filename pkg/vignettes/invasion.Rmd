...
```
  ...
  <p style="text-indent: 20px">If we remove `full = FALSE` from the `summary_hfv()` call, then our summary will reveal that the `verticalize3()` function has automatically subset the data to only those instances in which the individual is alive in occasion *t* (please look at the variable `alive2`). Knowing this can help us interpret other variables. For example, the mean value for `alive3` suggests very high survival to occasion *t*+1 (`r 100 * signif(mean(lathvert$alive3), 3)`%). Further, the minimum values for all size variables are zero, suggesting that unobservable stages occur within the dataset (these are instances of vegetative dormancy).</p>
...
```{r Ch2.17, fig.cap = "Figure 2.1. Image of mean ahistorical matrix"}
...
```{r Ch2.18, fig.cap = "Figure 2.2. Image of mean historical matrix"}
...
```{r Ch2.25, fig.cap = "Figure 2.3. Deterministic and stochastic ahistorical vs. historically-corrected stable stage distributions"}
...
```{r Ch2.26, fig.cap = "Figure 2.4. Ahistorical vs. historically-corrected reproductive values"}
...
```{r Ch2.35, fig.cap = "Figure 2.5. Ahistorical vs. historically-corrected deterministic and stochastic elasticity to stages"}
...
```{r Ch2.36, fig.cap = "Figure 2.6. Ahistorical vs. historically-corrected deterministic and stochastic elasticity to transitions"}
